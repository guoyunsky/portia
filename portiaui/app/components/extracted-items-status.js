...
export default Ember.Component.extend({
    ...
    changes: Ember.computed('extractedItems.changes', {
        get() {
    ...
    hasChanges: Ember.computed('changes', {
        get() {
            return this.get('changes').length > 0;
        }
    }),

    hasWarning: Ember.computed('type', {
        get() {
    ...
    icon: Ember.computed('hasWarning', {
        get() {
            return this.get('hasWarning') ? 'warning-triangle' : 'ok';
        }
    }),

    changesText: Ember.computed('changes.[]', {
        get() {
